...
For the character vector, attempting to calculate the mean results in an error: `r warning in mean.default(pull(frame_df,vec_char)):argument is not numeric or logical:returing NA`
For the character vector, attempting to calculate the mean results in an error: `r warning in mean.default(pull(frame_df,vec_factor)):argument is not numeric or logical:returing NA`

##Explanation for what mean works and what doesnot
*mean_numeric: This will calculate the mean of vec_numeric, which is a numeric vector. This will work because the values are numeric.
*mean_logical: This will calculate the mean of vec_logical. Logical vectors are treated as 1 (TRUE) and 0 (FALSE), so the mean will represent the proportion of TRUE values.
*mean_char: This will fail because the vec_char is a character vector, and r cannot calculate a mean for characters. 
*mean_factor: This will also fail because factors are categorical variables. and r cannot calculate a mean for characters. 
